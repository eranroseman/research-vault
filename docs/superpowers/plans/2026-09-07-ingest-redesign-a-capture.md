...
| `path-shim`          | the first `imported_file` row of `items?itemType=attachment` (every attachment, one unpaginated read — research record 115; filtered client-side) has its `/file/view/url` resolved through `paths.to_local` to an existing file | resolved path does not exist                                                               | zotero unreachable or no stored attachment among the first 50 | not running in WSL  |
...
**Two more rulings for fix round 2, from round 1's measurements.** The `limit=50` window on `path-shim`'s attachment listing was a sample that moves: on the same production instance an hour apart, one run surfaced a stored file and MATCHED, the next got fifty `imported_url`/`linked_url` rows and SKIPPED "no stored attachment to resolve" while stored files sat further down the library — a linter whose verdict flips with nothing changed in what it lints. The local API returns the full result set by default (research record 115: "Results are not paginated by default… `limit` and `start` still work if you want them"), so the probe reads `items?itemType=attachment&format=json` with no `limit`, once, and filters client-side as decision 25 says; the SKIPPED reason names how many attachments were inspected (`no stored attachment among N`), so a reader can tell an empty library from a linked-only one. And a `zotero_profile` value that is present but not a string is UNMATCHED `zotero_profile must be a string`, the same reading Task 17 gives `zotero_base` — SKIPPED stays for the absent or empty key alone.
...
def _doctor_fake(monkeypatch, tmp_path):
    ...
    fake.get("/api/users/0/items?itemType=attachment&format=json",
...
def _path_shim_probe(client, info, vault) -> Probe:
    ...
    try:
        ...
        payload, _ = client._local_json("/api/users/0/items?itemType=attachment&format=json")
