...
>
> **Sequencing, ruled 2026-09-01:** build now, not after `writing-specs`. `writing-requirements` never merges upstream — the delta is total — so it was never a drift surface, and copying `writing-specs`' skeleton later is copying the same upstream bytes one hop on. House deltas can flow the other way: `writing-specs` adopts the gate and the ritual inversion from here. **The gate on building is R27, not sequencing** — four corpora remain unapplied and their leads predicted three later-confirmed defects.
>
...
It does **not** close them for the skeleton, which is taken for its structure; the screen exempts structural takes from content fail-counts by its own words. What closes them there is availability rather than measurement: **`writing-requirements` is a new skill, so there is no upstream artifact to install, fork or copy.** Forking a repository to obtain one file you then rewrite is not a fork. What remains is **copy plus a delta**, **author to someone else's design and credit it**, or **write from scratch**. (The first five are the sourcing ladder drafted in `docs/2026-08-31-proposed-adr-software-development-component-adoption.md`, which is a proposal and not accepted. The sixth — author to someone else's design and credit it — is **not in that draft**; it was added in the 2026-09-01 conversation, along with a ruling that the ladder is not finalised. Names used for brevity, not as authority.)

### Adapt

| Source                                           | Licence    | What we take                                                                                                                                                                                                                                       | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `brainstorming` — obra/superpowers               | MIT        | The skeleton: the one-way ratchet, the HARD-GATE, the per-path checklist, the Anti-Pattern and Red Flags *forms*, the process-flow graph, and the surviving half of the After-the-Design ritual — self-review, user review gate, invoke next skill | Structural symmetry with `writing-specs`, which derives from the same upstream. **The three-path classifier is not taken.** Spike/Bounded/Architectural classifies code — "bounded" is defined by whether the flow already exists in the repo, and a need has none; a spike has no requirements to write. R2 supplies this phase's proportionality and it is binary. The **ratchet** is taken and retargeted: a run that started never downgrades to a decline, and a decline reverses when an answer shows the pre-flight was answered wrongly. The classifier stays right for `writing-specs`, where code exists — so the siblings differ in one section by design, not by drift. **Two of the four ritual steps are also not taken**: the checklist asks one question at a time, which R30 forbids, and After-the-Design writes at the end, which R8 forbids — the write moves to the top of the loop. **Vintage: HEAD as of 2026-09-01 (v6.3.0)**, recorded for the ratchet's wording; everything else taken here is present in the installed 6.2.0 |
