...
In every command, `PATH` is the vault and `CITEKEY` is the Better BibTeX key. Every mechanical act below is a CLI verb call: you compose and explain, the CLI writes. Never hand-write a literature note, a managed region, a `verified` event, or a review-inbox entry.
...
A source with a `url` and no `doi` is a web source, and web content rots. Rescue is impossible after the fact, so the snapshot has to exist **now**, at import — later detection cannot bring a dead page back.

At import time, for each such source: record an existing snapshot if one exists, or trigger a fresh capture (Internet Archive Save Page Now, `https://web.archive.org/save/URL`), then write the resulting snapshot URL into the note's frontmatter as `archive-url`. That field is pass-through metadata: the renderer does not own it, so it survives every later re-render, and it is not an event, status, tag, hold, or acknowledgment — writing it is not a CLI act. The `accessed` date is captured on day one and never overwritten.

The harness has no archive verb of its own, and its polite-pool request machinery is internal to the CLI's own checks — you are not calling through it, so do not claim you did. What the harness has is the check: `verify`'s `web-archive` check files a `missing-archive` finding when a web source has no `archive-url`, or when the recorded one no longer resolves. That check detects; it never captures. If a capture fails or you cannot reach the archive, say so and file nothing — an outage is not a snapshot, and a note with no `archive-url` will be reported by the next `verify` run anyway.
