...
08. factcheck-draft: per-claim report template + weighted rubric; plus the source-grounded coverage-QA pattern (adopted 2026-08-24 — closed-ended questions generated from the source, answered against the summary; the deepeval/ragas METHOD as a prompt pattern, no package) (two adjudications of one claim must not silently diverge); the receiving-code-review half (rigor-not-agreement when the person pushes back on an adjudication).

09. Worked-example norm: adopted per-skill above where cited, not as a blanket rule.

10. *(scope addition, ruled 2026-08-22 — slice finding 15)* import-source: explicit **ingest step** between §2 and §3 — the session agent authors a free-region digest as tagged claims per evidence-conventions (`(paraphrase)`/`(inference)` + citekey + locator; summary inside the verification perimeter). LLM only — no NLP library (lane boundary, §8 contract mismatch, extraction quality; see the finding). Compilation-value gate: a concise searchable source may need no digest; no-op legitimate. No auto-extracted quote candidates — quotes stay annotation-projected. **Generation is an isolated single-turn call (adopted 2026-08-24, evidence: multi-turn degradation ~39%, irrelevant-context distraction — research/prior-art/2026-08-24-summary-quality-evaluation-python.md §3): source text + instruction only, never mid-session** — which also eliminates cross-contamination by construction (a generator that sees only its source cannot cite anything else). The summary's first line carries `[summary-of:: <fixity-prefix>]` (adopted 2026-08-24) so source-artifact drift flags the summary stale through existing fixity machinery. The step ends by surfacing the digest to the author for emphasis guidance before the session moves on (Karpathy's "discuss key takeaways", as one sentence, not a gate). **Full-text precondition (author-ruled 2026-08-22): a digest is authored from the source's full text, always and only** — never from the abstract alone. Route: read the attachment directly (Zotero storage path via BBT `item.attachments`) until the `/fulltext` leg lands. No reachable full text = the digest slot states "no full text available" — that is the only honest summary in that case. An abstract-derived digest is fabricated facts (invented specifics attributed to the source), same zero-tolerance class as a fabricated citation; the abstract is the author's persuasion surface, and building on it inherits spin as fact.

11. *(scope addition, ruled 2026-08-22 — the skill-eval lane)* **Behavioral eval brackets the pass**: step 0 runs a baseline eval over the shipped nine BEFORE any edit; the final step re-runs it and diffs compliance per skill. Harness: the RED-phase design (pressure scenario → fresh agent → comparative judge, COMPLIED/FAILED + decisive quote); scenarios generated from each skill's rationalization-table rows + core discipline (2–3 per skill — the tables are pre-authored adversarial eval cases). Investigate `claude plugin eval` as the native carrier first; the RED-phase workflow script is the proven fallback. Honesty clause: n=1 per cell is directional — paraphrased repeats on any close result, and the eval gates edits without certifying skills. Rationale: `test_skill_contracts` pins form only; behavioral drift is the drift that matters, and prose edits are when it happens. Extended 2026-08-24: the harness also carries ROUTING fixtures (firing-eval — realistic phrasings asserted fire/stay-silent per skill, gbrain's `routing-eval.jsonl` precedent) — a distinct axis from compliance: does the right skill fire at all, and do the seven user-gated ones correctly stay silent.

12. *(prose-vs-mechanism audit 2026-08-24)* **synthesis-shape lint**: mechanize synthesis-conventions' three deterministic rules (2+-source threshold, min-link discipline, index registration) as a warn-tier lint — sharpened 2026-08-24: the source count collapses shared-origin sources (same author, one-cites-the-other) so only INDEPENDENT sources count (claude-obsidian's `_independent_group_count` precedent; this lands the long-parked independence-key deferral), and a fourth predicate joins the family: `confidence-present` (an inference claim in synthesis missing `[confidence:: ...]` is mechanically detectable) in `lint_screening_state`'s shape; the skill keeps only the judgment half and drops its lines 14–24.

13. *(same audit)* **Registry single-sourcing**: `REASON_CODES` becomes code→meaning dict; a `reasons` verb prints it; `validate_reason` enumerates the registry in its refusal; the skill's 17-row table becomes a pointer. Likewise `verify` prints its check-id roster + per-surface closure in the JSON summary; the four skill copies become pointers.

14. *(same audit)* **Shared-contract dedup**: the four four-state tables keep only their per-surface delta column — design settled 2026-08-24: the canonical table lives in `evidence-conventions` (the shared-vocabulary skill all four already route to), and the collapse is the moment "four-state honesty" becomes the one settled leading token (RED already unblocked the dedup); the 0/1/2/3 exit-code contract moves to one `--help` epilog with skill pointers; the six "never hand-write" statements collapse to one glossary statement + pointers.

15. *(same audit)* **`trust-tier --project NAME`**: the per-citekey loop the project skill runs by eye moves into the verb.

16. *(same audit — recorded tension, no action)* typed-discard consent stays prose: mechanizing it cuts against "deletion is not on the menu."

17. *(mechanization prior-art note, 2026-08-24)* `redact_url` runs unconditionally inside the `search-log` verb on every argument before the line is written — the eliminate rung; the skill's "by hand for anything you don't run through it" branch dies structurally.

18. *(same note)* token-ordered subsequence matcher (medsci's `_ordered_run` class) beside `selectors._norm_with_map` — catches mid-sentence superscripts/line numbers the contiguous find misses; a deterministic precision gain on the `quote` check that shrinks the LLM-adjudication surface by exactly the false-mismatch cases.

19. *(same note — INVESTIGATE first)* UNREACHABLE recheck: verify whether an aged outage finding auto-clears when the later re-run MATCHes (the scope/marker machinery may already cover it — `verify` re-runs every check per invocation, so the CHECK returns; the open question is only the inbox record). The research-hub-style recheck flag lands only if the gap survives the investigation.

20. *(Vale evaluation, ruled 2026-08-24 — sits beside item 13, the other registry single-sourcing item)* **Terminology guard as pytest**: a test parses `CONTEXT.md`'s `_Avoid_` lines at test time and fails on a rejected term appearing in `docs/` (mutable paths only) or `skills/`. No generated artifact — the glossary *is* the registry, read live, so drift is structurally impossible. Needs a paren-aware comma split (`import (that is the projection step that follows)` breaks a naive one), code-span stripping, and `\b`-anchored terms — the two false-positive classes measured at `research/prior-art/2026-08-24-vale-evaluation.md` §4. Append-only paths (`research/`, `analysis/`, `docs/adr/`) are globbed out: a finding there can be neither fixed nor annotated away. A guard on skill and doc prose, not slice-gating. Vale was evaluated as the alternative and declined on the rung ordering; that note carries the verdict and its revisit triggers.
...
Source: `research/validation-slice/2026-08-24-post-q-batch-review.md` (independent review) and the per-task reviews of the `fix/pre-slice-batch` run. Triage at the polish pass; none blocks merge.
...
- **T1** — stale `project` references survive in `docs/product-landscape/2026-08-22-adoption-plan.md:585`, a self-declared *living* document, which the write-once exemption does not cover.
- **T2** — nothing binds `ENTRY_SKILLS` to the vault AGENTS.md index in reverse, so an eighth entry skill goes missing silently; the `publish` row is additionally invisible to `_BACKTICKED_KEBAB_TOKEN`, which requires a hyphen.
- **T3** — `tests/test_skill_contracts.py:389` — `_COUNT_WORDS[len(exempt)]` raises an opaque KeyError for 0 or >5 exempt codes, and the leg pins the sentence's phrasing rather than its contract.
- **T3** — `:225` — the AST scan reads only positional first args and only `ast.Assign` module constants, so a future `Outcome(check="new-id", ...)` silently drops an id. Both blind spots verified latent at HEAD.
- **T3** — `:369` — the C-2 pin's docstring claims moving `manual` off the table fails the equality; it does not. The pin proves accounting, not placement.
- **T3** — `:339` — module docstring promises nothing is scoped to a particular skill, but the reason-code pin hard-codes `evidence-conventions/SKILL.md`.
- **T3** — `evidence-conventions/SKILL.md:88` — the `manual` row's "the only code in this table a person writes" is unmandated, unpinned prose; `inbox.py:381-384` makes every ack human-actored carrying some registry code.
- **T4** — `tests/test_find_sources_vendor.py:223` — the AST helper recognises only `os.environ.get`/`os.getenv` with a literal first arg, so `os.environ["NAME"]` or a bare `getenv` would be missed and the set-equality still pass.
- **T4** — `find-sources/SKILL.md:70` — Step 5 and guard 2 both say to run a URL through `redact_url`, but `scripts/_common.py` is import-only, so the skill gives no way to execute the instruction it gives.
- **T5** — `setup-vault/SKILL.md:21` — the kept-six clause reads "Paths such as …", reopening a set the brief closed, and two entries are directory prefixes scaffold never prints as created paths.
- **T6** — `import-source/references/batch-mode.md:9` — "the same per-note contract as above" no longer resolves; the "above" was §7, now a sibling file.
- **T6** — `import-source/SKILL.md:96` — the §7 lead-in carries no read-before-acting imperative, so the archive-at-import mandate sits behind a pointer with nothing telling the reader to take it.
- **T6** — `:100` — the three new pointers are unpinned; no test asserts the table's paths resolve.
- **T7** — `find-sources/SKILL.md:74` — a sixth spelling of the never-hand-write refrain survives ~17 lines above the collapsed `:91`. CONTESTED, not plan-mandated: the audit's five-site set was drawn with this line already in the tree.
- **T7** — `publish:11` and `import-source:13` stack three em dashes doing two jobs. Style only; every enumerated noun survives.
- **T7** — the task-7 report claims the audit "praised project-flow as the closest to ideal" — no source in the audit.
- **T8** — `synthesis-conventions/SKILL.md:18` — the compilation-value line ships unpinned because no test reads that file; pinning needs a new pin file. §7 polish item 06 is queued to rewrite this prose anyway.
- **T8** — `publish/SKILL.md:138` — the refusal row omits the resolve/acknowledge clearing path that `:61` and `hooks/stop_publish_gate.py:535` carry. CONTESTED: nothing in the row is false, but the enumeration is narrower than `:61`'s.
- **T8** — `publish/SKILL.md:137-139` — three table rows repeat their source paragraphs near-verbatim, so editing the canonical prose leaves the table asserting the old rule with the suite green.
- **T8** — `publish/SKILL.md:11` — the gate announcement omits the eight-block safety bound (`MAX_ACTIVE_BLOCKS = 8`, released at `:506-507`) after which the session ends with the gate still armed; and "is acknowledged" attaches the ack to the attempt rather than to a blocking finding.
- **T8** — `tests/test_publish.py:815` — the rationalization pin asserts against the whole file instead of slicing to the section, so a row moved out of the table still passes.
- **T9** — `tests/test_inbox.py:877, :890-892, :898-899` — three comments restate the assertion beneath them instead of stating a constraint the code cannot show.
- **T9** — `tests/test_verify_cli.py:1163` — the dedup guard has no positive control, so it cannot distinguish "dedup suppressed the filing" from "nothing would have been filed anyway".
- **T9** — `:1181` — the guard's only assertion reads through `inbox.open_entries()`, the function it protects, so its failure points at the reader rather than the duplicate row.
- **T10** — `tests/test_checks.py:81` — the scope bound (a note's OWN citekey row keeps note-vs-bibliography semantics) ships with no committed regression test; one was written, run, and deleted.
- **T10** — `checks.py:159` — the `literatures/<citekey>.md` layout convention is now inlined a fourth time (`notes.py:129`, `lints.py:491`, `factcheck.py:73`/`:90`) with no owner.
- **T10** — the tracked mutation sidecars for `checks.py` and `inbox.py` are stale; hash-verified. Per controller ruling, sidecar refresh is a baseline act, not a task act — Plan W's baseline retires them wholesale.
- **T2b** — the canonical `index.md` literal is duplicated verbatim in `tests/test_scaffold.py` and `tests/test_templates.py` and must be hand-edited in lockstep. (Upstream has since consolidated these; re-check at the polish pass whether this still holds.)
- **T2b** — nothing asserts the `index.md` → `system/bases/*.base` embed targets actually resolve in a scaffolded vault; the paths are pinned only as opaque text.
- **T2c** — the template's "Machine surfaces (…) are owner-written" line still diverges from the preamble's four-surface list — omits `log.md` and `literatures/`, adds managed regions and `system/bibliography.json`.
- **T2c** — those are two enumerations of machine surfaces with different membership and different consequence clauses, and nothing marks which is the abridged form.
- **T2d** — `tests/test_templates.py:317` — the formatter-ignore pin uses substring and aggregate assertions, so a commented-out or negated pattern, a deleted `root = true`, or lopsided property placement all keep it green. **Redesign, not a wording pass.**
- **T2d** — the `.editorconfig` header's rationale is restated near-verbatim in the test comment; two copies will drift.
- **NEW** — `oldest_age_days` goes **negative** for future-dated findings; reproduced at −26428. Code landed this session (`eff8d67`).
