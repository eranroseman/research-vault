...
| Result      | Meaning                                                                                                                                                                                                                                           |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MATCHED     | The check ran and agreed. **Only `update-notice` and per-claim `quote` mint a `verified` event on MATCHED** — every other check id the run reports leaves no separate durable trace beyond the run's counts. You never write an event either way. |
| UNMATCHED   | The check ran and disagreed. Already recorded in the review queue by `verify` itself — do not also file it yourself.                                                                                                                              |
