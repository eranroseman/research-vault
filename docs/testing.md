...
No offline test opens a socket to Zotero: the autouse `tests/conftest.py::_no_zotero_socket` fixture makes every `ZoteroClient` read outside the `live` markers an outage, so the offline suite gives the same answer on a machine with no Zotero and on one where a production instance is running; a test that needs a Zotero answer registers it on `tests/fakes.py::FakeZotero`. Under it, offline runs are socket-blocked: the autouse `_no_socket` fixture raises on any TCP connect, naming the address, so no client class can reopen the hole; a test that needs the network carries a `live` or `live_net` marker (the block is off entirely when either flag is set). Both blocks are in-process patches: a test that launches the CLI in a subprocess is hermetic only through the `dead_base` fixture — an ephemeral loopback port nothing listens on, refused at once — which is also the one address the in-process block lets through.

`RV_LIVE` unlocks the local-Zotero legs; `RV_LIVE_NET` the external-registry legs (the mailto rides the polite pools — Crossref etiquette). The one skip remaining after both flags is `tests/test_config_validity.py::test_offline_tests_cannot_open_a_tcp_connection`, which asserts the socket block itself and is skipped while the block is off. Gated tests are invisible to offline suite-green — after renames or seam moves, run the live legs before claiming the wave complete.
