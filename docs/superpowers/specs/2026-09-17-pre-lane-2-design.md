...
33. **The third going-public precondition — assembly spec §12's unlicensed verbatim text and the author's home paths in the tree — was missed before the flip and lands at once, the repository staying public** (§3.10; added 2026-09-18, #164, operator's choice).
...
Every run since `aa75c42` fails at "Form + lint" — ruff `PERF401` at `tests/test_current_state.py:136`, `FURB167` at `:142` — and skips every later step. Fixed in the plan's first task.
...
- 120 Claude Code session-transcript blobs (the largest ~32 MB) — at `research/raw/knowledge-harness-transcripts/` from `7cacace`, under `docs/research/raw/` as `knowledge-harness-transcripts/` and later `research-vault-transcripts/` from `d0e7800`, removed from the tree at `b275f1f`.
- Eight PDFs, one `.txt` extraction and a `README.md` once tracked under `sources/`.

A scan of every transcript blob for secret-shaped tokens found none. Contributor state (`wiki/`, `.raw/`, `.vault-meta/`) never entered history (`git log --all --diff-filter=A`, empty, 2026-09-17). The first commit touching the paths above is `4847b80`; every later commit is renumbered, and both `origin/prototype/*` branches reach the same blobs.
...
3. **Commit the map** — `.git/filter-repo/commit-map` becomes `docs/agents/history-rewrite-2026-09-17.tsv` (to be created; old, new, one line each) — and rewrite through it: every sha-shaped token in any tracked text file that resolved before the rewrite (98 in Markdown at `5a94c77`, plus the ones this spec and its sibling added; outside Markdown, `tests/test_skill_contracts.py:492`, `research_vault/capture.py:219`, `research_vault/captured.py:265` and `.github/workflows/quality.yml:53,87`), and every issue body and comment citing one (35 issues; tokens the map has no entry for — run ids, transcript hashes, foreign shas — are left alone). The same pass sweeps the old-slug URLs (§9.2). Commit messages keep their old shas.
...
The Git line under `### Git` becomes: *Push the branch; when its required check is green, `git merge --ff-only` it into `main` and push. A `--no-ff` merge mints a sha no check has run on and the push is refused; a branch behind `main` cannot fast-forward — rebase, push the branch again, wait again. Fetch before claiming something is absent from the remote.*
...
### 3.10 #164 — the third precondition the rewrite does not remove (added 2026-09-18)

**Finding.** Assembly spec §12 names three preconditions that block the flip; §3.1 cleared the first two (the transcript blobs and the `sources/` files leave history through the rewrite) and did not name the third: *the current tree carries what neither route removes* — `docs/research/2026-09-04-import-sourcing.md` reproduces verbatim text from candidate repositories that record no licence (all rights reserved by default), and tracked files carry the author's absolute home path. Measured 2026-09-18 at `0c1106e`: `git grep -lE "/home/[a-z]+"` returns ten files (sixteen lines) — the sourcing record, three research records, two dated plans, two specs, and the two `scripts/mutmut_shims/` docstrings; the record's own `**Licence.**` verdicts at that commit read *None found* for §10 (Karpathy's gist) and §12 (the llmwikis.org bundle), *declared but incompletely papered* for §21, and *split* for §11, against 35 candidates with a licence found. The repository went public at Task 5 (2026-09-18) before the omission was noticed (#164, filed at the close of #46); the exposure is live, and the rewritten history carries the same text.

**Design.** The operator chose to stay public and land the fix at once, as the next task after the one in flight (decision 33). The record's own verdicts are the rule: every fenced verbatim quotation of a candidate whose licence is not found at the pin becomes its citation plus a paraphrase in the note's own words; quotations from licensed candidates stay, their licence named in the candidate's Licence line; the header's conventions paragraph says which candidates are quoted under which licence and which are cited-and-paraphrased. The record keeps `Disposition: historical` — the edit is the removal its own precondition asks for. The home path is swept from the ten files to `~/…` wherever the path carries information (a plugin-cache path, a foreign checkout, the pilot report) and removed where it does not; the two shim docstrings are prose, not a default or a fallback (measured: neither file reads the path). Mechanism: a test in `tests/test_current_state.py`'s shape over every tracked text file asserting no `/home/<user>` literal, so the class cannot return; the licence question has no mechanism beyond the header sentence. A second history rewrite is not in this spec: it is a separate operator decision.

**Pins.** `test_no_home_directory_literal` fails on the sixteen lines before the sweep and passes after; the task report counts the candidates reduced and the fences removed. Decision 33.
...
| --- | -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| M1  | A duplicated top-level key is observable after parse                                                                             | `test_literature_notes.py:104-116`; `test_stamp.py:43-52`                                                               | `literature_notes.validate_managed_witness` (`:118-127`); `stamp.py:114`                                                             |
| M2  | A duplicated key inside an inline `{…}` mapping fails closed                                                                     | `test_frontmatter.py:90-99`; `test_events.py:458-486`; `test_literature_notes.py:278-330`                               | `events.py:49,89`; `literature_notes.py:177`                                                                                         |
| M3  | The body is returned byte-for-byte — no strip, CRLF kept                                                                         | `test_frontmatter.py:41-48`; `test_literature_notes.py:235`; `test_events.py:60-84`                                     | the `managed-sha256` witness (`literature_notes.py:98-131`); `render_note` (`:502`)                                                  |
...
| M5  | Key order: source order on parse, insertion order on dump                                                                        | `test_literature_notes.py:416-445`; gap — the order-exact header comparison (`test_inbox.py:99-114` pins presence only) | `inbox.py:278`; `searchlog.py:100`; `literature_notes.py:475,507`                                                                    |
| M6  | An unterminated block raises, never reads as "no frontmatter"                                                                    | `test_literature_notes.py:74-77`                                                                                        | `stamp.py:90-92` (`unparseable`); `captured.py:55-59`, `lints.py:91-96,577-588` (fail closed); `publish.py:138-140` (skips the file) |
| M7  | No YAML 1.1 coercion of unquoted scalars; control characters refused on write                                                    | `test_frontmatter.py:75-87`; `test_render_neutralization.py:47-76`; gap — `_parse_scalar` has no direct test            | `literature_notes.py:120-129,174-183`; `events.py:47-55`                                                                             |
| M8  | The boundary grammar is exact and shared with the byte-surgical writers (`----`, `---  ` are not boundaries)                     | gap — no test feeds either                                                                                              | `literature_notes.py:155-158`; `events.py:172,180-191`; `stamp.py:38-39,122`                                                         |
...
**Finding.** `lints._frontmatter_attestation_outcomes` (`:650-676`), `_write_attested` (`:613-630`) and the added-note leg (`:733`) read frontmatter through `data.get(key)`, which is last-key-wins, over `CAPTURE_FIELDS` (29 keys, `literature_notes.py:64-68`, aliased at `lints.py:570`). Four shapes evade at `HEAD`:
...
**Residual 3 — a nested directory under `literature/`.** `verify.py:1057-1062` walks the vault and `structure.check_note_frontmatter` types each file by folder, so `literature/older/x.md` gets one `okf-frontmatter MATCHED` row while every literature reader ignores it (flat since `4c6eca9`). **Design.** The walker stays. `structure.check_tree` returns a list — the existing tree row plus one `UNMATCHED — literature/ is flat: literature/<dir>/` row per stray directory; `verify.py:1064` extends; the id stays `tree`. `test_verify_cli.py:3223-3264`'s `okf-frontmatter` assertion (`:3259-3263`) gains the row.
...
**Row 11 — closed on evidence:** `tests/test_structure.py:144-166` already scaffolds with `scaffold_vault` and runs `verify_state` over the result. **Row 43 — closed on evidence:** fixed at `962666c`, renamed at `663d4cf` (`tests/test_scaffold.py:608-642`).
...
- `_read_note_text` (`verify.py:62-64`, in `markers.py` after the cut), guarded once for its five unguarded callers `verify.py:603,698` and `publish.py:373,467,479` (its other five call sites already catch `OSError`/`UnicodeError`); `capture.py:187`.
...
### 7.6 #163 — ADR 0001 severed: the decision stays, the mechanism moves (added 2026-09-18)

**Finding.** `docs/adr/0001-vault-outlives-its-tools.md` carries three layers — the decision (the vault stays readable, navigable and adoptable if research-vault disappears, through a named external mechanism), the mechanism (OKF conformance, pinned; §11's three rules verbatim; the reserved-file rule; two exemptions; the adopted families; the attribution deviation) and the options and consequences — at 61 lines against the 13–33 `docs/agents/domain.md`'s ADR rules describe, and it is edited whenever the mechanism moves (the pin, an exemption, a family) though the decision has not moved since 2026-08-20; its own Consequences say the two are severable. `domain.md`'s routing doctrine postdates the ADR and the ADR breaks it: rules agents follow go where agents read them; an ADR is only for a choice a future designer would otherwise reverse without knowing what it cost. The conformance picture also has no home: five consumers (`research_vault/okf.py`, `research_vault/structure.py`, doctor's `tree` probe, the weekly `okf-spec-drift` job, the pin) and two registers — the ADR's exemptions, and `docs/agents/terminology.md` §3, whose one table mixes vocabulary deviations (a term's sense or spelling declined against an anchor: `source`/`venue` against OpenAlex, the pandoc attribution form) with conformance deviations (an OKF rule or field shape the vault does not meet: frontmatter on `wiki/index.md`, untyped `inbox/` captures, `verified[].check`, the date-valued `at`, the `[[key#^claim-id]]` link form, untyped lineage for `supports`/`disputes`, no `stale_after`, no §10 attested computation). Nothing mechanical reads §3 (`tests/test_config_validity.py` reads §4.4; `tests/test_current_state.py` reads §5).

**Design.** The ADR keeps the decision; one paragraph naming the mechanism without describing it — OKF conformance, pinned, the pin in `.github/okf-pin.json` (§7.5), the rules, their discharge and the deviations in the conformance document; the scope bound (the vault preserves the record, not evidence artifacts); the considered options; the severability consequence. Not less: an ADR that only says "the vault survives the tool" is the rejected "no named mechanism" option in disguise. `docs/agents/okf-conformance.md` (to be created) carries OKF §11's three rules verbatim and the reserved-file rule; the families adopted where adoption is additive (actor prefixes, trust tiers, the event shape); how conformance is discharged (`structure.py`, `okf.py`, `stamp.py`, doctor's `tree` probe, the drift job reading the pin file); and the conformance deviation register — the eight OKF-rule rows moved from terminology §3 with the same three columns and terminology §1's cost classes. Terminology §3 keeps the four vocabulary rows (the pandoc attribution form, `citation key` against `sources`/`resource`, the project lifecycle words, `source`/`venue`) and §4.6's adopted spellings, and gains the routing rule so the two registers do not merge back: the declined anchor decides — an OKF rule, section or field shape registers in the conformance document; a term's spelling or sense registers in `terminology.md`; both cite §1's cost classes. Citers repointed: `research_vault/structure.py:37` ("ADR 0001, second exemption" → the register row), the drift job's issue body in `.github/workflows/quality.yml` ("Reconcile per ADR 0001" → the conformance document), `ATTRIBUTION.md`, the ADR's own link to the register; `AGENTS.md` gains one line under Agent skills, as `sourcing.md` and `testing.md` have. Lands with §7.5 in one commit — same task, same review: the ADR trimmed, the document created, the rows moved, the citers repointed, the `AGENTS.md` line — never in pieces, or the two registers coexist and the ADR points at a file that does not exist yet.
...
6. **`factcheck-draft:36-42` reads the literature note's "body"**, which carries no source text (`literature_notes.py:407-437`); the text is `fulltext/<attachment key>.md`. Named; the skill gains the untrusted-content sentence wiki-query (`:11-15`) carries.
...
Two repairs ride the edit: `:272` still says `export_csl(None)` "is live in the tree today" (removed at `80a7a56`); `sourcing.md:37` gains a dated sentence pointing at the paragraph as the record. `zotero.py`'s docstring points at it.

### 9.2 #91 — the identity rename, closed

**Finding.** Done at `dab35a9` except `pyproject.toml:9`, which reads `name = "research-vault"` on the strength of a terminology row (`df45a6b`, an hour before #91 was filed; collapsed at `17c8c3a`, dropping the PyPI sentence) that never cited #91. PyPI, 2026-09-17: `research-vault` taken, `research-vault-core` free. The pip-install label lives in `research_vault/templates/ci/verify.yml:18` and `rw-batch.yml:23`, pinned at `tests/test_templates.py:296-297, 336-337`. 35 issues still carry the old slug (13 bodies, 26 comment threads).
...
02. Going public — §3.1 steps 1–5, the ruleset last; §3.2's `.gitignore` and test; §3.7's amendment. §3.10 (added 2026-09-18) belonged before step 4 and was missed; it lands as the next task after the one in flight when it enters the plan (Task 20b), the repository staying public.
...
| #129  | §4.5, §7 | rows 11 (`test_structure.py:144-166`), 43 (`962666c`) and 24's dead port (`b10e86d`) closed on evidence; row 24's fixture ids on the Part B ruling; row 25's quirk 3 as not matching the code; every other row and item designed |
| #136  | §5.1     | decision 15                                                                                                                                                                                                                      |
| #20   | §5.2     | decision 16                                                                                                                                                                                                                      |
| #107  | §7.3     | decision 24                                                                                                                                                                                                                      |
| #32   | §7.4     | decision 25                                                                                                                                                                                                                      |
...
**Amendments that rode this spec's commits** (`330d0b1` onward): the ingest spec's `:3` became an execution record (executed as lane 1; binds until superseded; amended from here); the assembly spec gained the 2026-09-17 State line at `:297`, the decision 22 amendment at `:105-107`, the corrections and supersession at `:101`/`:103`, and the "settled by" sentences at `:573`/`:579`; `AGENTS.md`'s boundary section gained its purpose line (`303ce5b`).
