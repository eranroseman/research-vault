...
The register recommends **coexistence, not substitution** — item 09 reads *"diagnosing-bugs **beside** systematic-debugging"*. Its looser win-section line calls the loop-first approach *"arguably a better fit for this repo's measurement doctrine than systematic-debugging's four phases"*, but that compares approaches rather than proposing a removal.

A complementary option, cheap and licence-clean: the fork puts `skills/systematic-debugging/` under the author's control, so `building-a-feedback-loop.md` could sit beside `root-cause-tracing.md` carrying the Phase-1 ladder and the red-capable criterion, with provenance. That transfers the *method* but not the *gate*, complementing hard-gated adoption rather than replacing it.

*Replacement is available and not recommended — yet.* The fork could delete `systematic-debugging` outright and give the slot to `diagnosing-bugs`, ending the trigger race instead of gating around it. Two edits would follow in a file the patch already touches — `using-superpowers/SKILL.md:31` routes *"'Fix this bug' → superpowers:systematic-debugging first"* and `:28` names it a most-common process skill — plus a cosmetic dangle at `writing-skills/SKILL.md:284`, which cites it as a style exemplar. **The blocker is an unrun check:** this survey established what the spine lacks, never what it has that `diagnosing-bugs` lacks. `systematic-debugging` ships `root-cause-tracing.md`, `defense-in-depth.md`, `condition-based-waiting.md`, `find-polluter.sh` and a worked example, none of which was compared. Recommending replacement on one-directional evidence is the error pattern this survey has already had to correct several times.
