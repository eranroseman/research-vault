...
**No frontmatter route.** An earlier draft of this section put the marker in a `disposition:` frontmatter key for files opening with YAML, and gave the count as "all 23 `skills/*/SKILL.md`". Both were wrong, measured 2026-09-05: `skills/` holds 23 `.md` files of which **9** are `SKILL.md` with frontmatter, the other 14 being `references/*.md` with none. And the route itself is disqualified twice over — a `disposition:` key would ship a repo-internal marker into the plugin surface a user installs, and it would break `tests/test_skill_files.py:20`. All 9 `SKILL.md` carry a heading immediately after their frontmatter, so the body anchor reaches every file in scope and the pass needs one rule rather than two.
