...
- Content sources referenced by task: `docs/research/validation-slice/2026-08-22-skills-layer-audit.md` (the C-findings and ecosystem steals), `docs/research/validation-slice/2026-08-22-references-cross-read.md` (items 11–12's itemized annotations). Copy their itemized content verbatim — the triage there is the decided set.
...
- [x] **Step 1: Metadata row (§6, ~line 98)** — append: *"A compared field absent on either side is SKIPPED for that field, recorded in the outcome's detail — never folded into MATCHED (closed 2026-08-22, audit gap)."* Then verify the implementation agrees (read `check_metadata`'s absent-field handling; if it silently folds, fix it with a failing-test-first micro-cycle inside this task and note it).
...
- [x] **Step 5: Commit** `fix: close spec §6 missing-data gaps; trust-core remediation acceptance` — then merge to main and push in the same motion.
...
**What is named:** the authored prose account of what one source says, written into that source's literature note free region — authored (salience judgment), never machine-projected. Ruled into existence by slice findings 15–16 (`docs/superpowers/specs/2026-08-16-foundation-spec-validation-slice-results.md`), still unbuilt: no skill step writes it today (Plan D polish-pass item 10, gated on the deferred `/fulltext` leg).
