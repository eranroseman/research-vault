...
The publish surface closes on `citekey`, `evidence-layer`, `quote`, and `update-notice`. Exit `0` means green, `1` means a closing check is UNMATCHED, `3` means something was UNREACHABLE, and `2` means verification could not run at all.
