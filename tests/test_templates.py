...
def test_markdown_templates_match_canonical_content():
    ...
    assert asset("vault/system/glossary.md").read_text() == (
        ...
        "a real source, verified by mechanical checks. This page defines the "
        "words this vault's notes, folders, and fields use.\n"
        "\n"
        "## Vault\n"
        "\n"
        "**Vault**: A private git repository of markdown notes — the "
        "researcher's durable knowledge store, packaged to survive its tools "
        "(currently as an OKF — Open Knowledge Format — bundle).\n"
        ...
        "\n"
        "**Synthesis layer**: The LLM-maintained pages (`synthesis/`) that "
        ...
        "\n"
        "**Log**: The append-only per-day activity record (`log/`), summarized "
        "in root `log.md`.\n"
        "_Avoid_: calendar, journal, daily notes folder\n"
        "\n"
        "**System folder**: The vault's support artifacts (`system/`): "
        "templates, bases, the bibliography export. Sorts last, out of the "
        "knowledge folders' way.\n"
        "_Avoid_: x (old name), assets, meta\n"
        "\n"
        "**Managed region**: The bridge-regenerated span of a literature note "
        "between `%%rv-managed%%` markers; never hand-edited.\n"
        ...
        "\n"
        "**Citable**: What a claim is allowed to cite: an item admitted in "
        "Zotero whose literature note exists here and is neither excluded nor "
        "superseded — being in the library is not yet being citable here.\n"
        ...
        "\n"
        "**Import**: The machine projection of an admitted item into the "
        "evidence layer — a literature note rendered from Zotero, never "
        "authored.\n"
        "_Avoid_: admission (that is the human act before), sync\n"
        "\n"
        "**Bibliography export**: The universe of items a citekey can name: the "
        "Better BibTeX auto-export at `system/bibliography.json`, written only "
        "by BBT.\n"
        "_Avoid_: bibliography file, reference list, citation universe (that is "
        "the evidence layer)\n"
        "\n"
        ...
        "## Verification\n"
        "\n"
        "**Check**: One named verification a note or claim is put through; most "
        "are mechanical, some are LLM judgment.\n"
        "_Avoid_: test, validation\n"
        "\n"
        "**Four-state result**: A check's outcome: MATCHED, UNMATCHED, "
        "UNREACHABLE (could not run — never guilt), or SKIPPED (does not "
        "apply).\n"
        "_Avoid_: pass/fail, pytest vocabulary in vault prose\n"
        "\n"
        "**Verified event**: The dated, attributed record of which check "
        "passed, appended to a note; only MATCHED mints one.\n"
        ...
        "(`inbox/review-queue.md`) every warn, hold, and alert writes to; "
        "drained at project orientation.\n"
        "_Avoid_: issue list, warning log\n"
        "\n"
        "**Acknowledgment**: A human's standing, hash-scoped acceptance of a "
        "finding — the recorded decision that lets a check stand down without "
        "the finding being erased.\n"
        "_Avoid_: dismissal, override (an ack keeps the record; it never "
        "deletes)\n"
        "\n"
        "**Publish gate**: The fail-closed verification boundary every "
        "publication crosses.\n"
        "_Avoid_: release check, CI gate (CI is the async auditor, not the "
        "gate)\n"
        "\n"
