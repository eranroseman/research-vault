...
**A defect in the sharing split, found by mapping those references — and it is narrower than it first looked.** Bare-name calls are fine within a product; the question is what happens when one crosses the boundary. Two of `wayfinder`'s callees do, and they are not the same case:

- **`domain-modeling` — load-bearing.** Called at three sites, all unconditional: the default ticket type (*"Always call the Skill tool twice"*, line 79), the mandatory first act of charting a map (*"Name the destination. Call the Skill tool twice"*, line 111), and the fallback at line 124. **`wayfinder` is shared, so `domain-modeling` must be shared** — that is forced by the call graph, not a judgement about which product wants it, and it settles the "genuinely split" question this survey could not otherwise resolve.
- **`prototype` — optional.** One site, reached only when a prototype ticket is created, which is one of four ticket types. Its absence degrades `wayfinder` to three types rather than breaking it, and a researcher plausibly never reaches for *"a throwaway prototype... UI/logic code"*. Tolerable, but the degradation should be stated somewhere the user sees it rather than surfacing as a Skill-tool error the agent quietly routes around.
