...
24. **A purge runs after each lane, including lane 0 — and it covers everything, not just code.** Not one sweep at the end: a disposition depends on which steps survive, so the lane that decides a step disposes of what served it while the reasoning is still in hand. **Lane 0's purge ran on 2026-09-07** — heavy rather than light, before the criterion below was settled: 28 issues, 11 documents, the 2,033-line disposition system, the SDD workspaces, the Scriptorium harness material, and a third of `CONTEXT.md`. It is over-delivered, not pending, and the plan slot is free. The 21-verb code audit it was originally sized for is exactly the hunt the criterion below rules out until lane 5.
...
| 3a   | Obsidian vault creation and seam-free plugins                        | opens by creating the vault that does not exist (§6.2); unblocks T5; **inherits the scaffold cost** — `scaffold.py` runs roughly three git subprocesses per seeded file, tolerable at 15 templates and not at a seeded `.obsidian/`, so lane 3a's spec owns it (closed #127) |
...
23. **Also found, and out of scope for this spec** — all four now dispositioned, none carried here. **#125** (mutation gate never fires) and **#126** (OKF pin written twice) are issues, because no spec will claim them. The scaffold subprocess cost is **lane 3a's**, named in §7.3 — filed as #127 and closed on the rule that the tracker is for findings with no home. The CRAP ceiling is decision 21's dated deferral. Kept for the record: the mutation gate has never evaluated a mutant, because its only firing condition is a pull request and this repository has none; the OKF pin is written twice, in `quality.yml` and in ADR 0001, with nothing comparing the copies; scaffold spawns roughly three git subprocesses per seeded file, which the seeded `.obsidian/` multiplies; and the CRAP ceiling fails on two functions, `_bump_generated` (43.1) and `check_metadata` (37.0), against a limit of 30 — **that one is no longer an issue to open; decision 21 carries it as a dated deferral.**
