...
No offline test opens a socket to Zotero: the autouse `tests/conftest.py::_no_zotero_socket` fixture makes every `ZoteroClient` read outside the `live` markers an outage, so the offline suite gives the same answer on a machine with no Zotero and on one where a production instance is running; a test that needs a Zotero answer registers it on `tests/fakes.py::FakeZotero`. Under it, offline runs are socket-blocked: the autouse `_no_socket` fixture raises on any TCP connect, naming the address, so no client class can reopen the hole; the marker is the mechanism, per test: a `live` or `live_net` test keeps the real transport, and every unmarked test is blocked in every run, live flags or not. Both blocks are in-process patches: a test that launches the CLI in a subprocess is hermetic only through the `dead_base` fixture — an ephemeral loopback port nothing listens on, refused at once — which is also the one address the in-process block lets through. The same class, for the operator's home: the autouse `_per_test_home` fixture points `HOME` and `XDG_CONFIG_HOME` at a fresh directory holding only a synthetic git identity (as global config, so a local `user.name` a test sets still wins), for the test and every subprocess it launches, so nothing a test reads from `~` — doctor's plugin registry, git's global excludes, a credential helper — depends on the machine (measured 2026-09-14: a mutant of the registry read died on the developer's machine and lived on the runner). Same gate: a `live` or `live_net` test keeps the real home, because the operator's registry and profile paths are what a live leg reads; the session-scoped template vaults are built under a home of the same shape.
