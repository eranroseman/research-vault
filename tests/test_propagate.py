...
def _planned(vault, mapping=(("old2020", "new2020"),), now=None):
    planned, outcomes = propagate.plan(vault, None, dict(mapping), now=now)
...
def test_lint_follows_a_key_renamed_back_and_reports_only_the_latest_mapping(
    ...
    propagate.apply(tmp_vault, None, back_path, back_digest)
    assert (tmp_vault / "literatures" / "old2020.md").is_file()
    assert (
        "[@old2020, p. 3]"
...
def test_plan_refuses_to_rename_over_an_existing_note(tmp_vault):
    ...
    planned, (refused,) = propagate.plan(tmp_vault, None, {"old2020": "new2020"})
    assert planned is None
    assert refused.result is Result.UNMATCHED
