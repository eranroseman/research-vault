...
research_vault/` clean on every touched file. Form gate
(`echo '{}' | python hooks/stop_publish_gate.py`) silent, exit 0.

## What was implemented

- `research_vault/notes.py::_split_free`: the guard changed from
  ...
  line matched the exact standalone `%%/rv-managed%%` close marker — now
...
- No change to `research_vault/__main__.py`: its
...
the controller's `research_vault/`+`tests/`-scoped grep) finds exactly one
...
`grep -rn "render_note(" research_vault/` finds exactly one production
caller: `research_vault/__main__.py:270`, inside `cmd_import_note` — the
...
- **Pre-existing, unrelated lint failures elsewhere in the tree.**
  ...
  modified (`research_vault/notes.py`, `tests/test_notes.py`,
