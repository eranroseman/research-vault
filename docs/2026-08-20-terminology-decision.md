...
- Interior coinages (`atlas`, `efforts`, `literatures`, `trust tier`, four-state names, `claim address`) are **decided, not settled** — renameable at churn-only cost until the first real vault mints them into git history, after which deprecate-never-delete makes recorded vocabulary permanent.
