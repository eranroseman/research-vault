...
- R2 (`caller`: vault end-user; `adr` 0001)
  [the vault glossary](https://github.com/eranroseman/knowledge-harness/blob/4f07dab0ca54f8a20a6da337cb3e478b15146d87/research_vault/templates/vault/system/glossary.md)
...
| R8 — accept only after review                      | ADR 0005 was committed with `Status: accepted` directly (commit `fd2f81f`), with no `proposed` stage                                                                                                                                                                                                                          | Caught within the same session, but disposal sits uncommitted (`git status` shows `D docs/adr/0005-apply-open-standards-at-applicable-boundaries.md`) — unresolved limbo, not yet a closed gap      |
