...
def resolve_snapshots(
    ...
    *,
    ...
    if candidate == "index":
        ...
        descriptor, private_index = tempfile.mkstemp(
            prefix="research-vault-index-snapshot-"
        )
        try:
...
_SCRATCH_PREFIX = b".research-vault-projection-"
...
def _temp_name(parent: bytes) -> tuple[int, bytes]:
    for number in range(1000):
        name = os.path.join(
            parent, f".research-vault-projection-{os.getpid()}-{number}".encode()
        )
        try:
...
def _restore_image(vault_root: Path, raw_path: bytes, image: FileImage | None) -> None:
    ...
    if image is None:
        ...
        temporary = absolute + b".research-vault-rollback"
        try:
    ...
    if resolved.exists():
        ...
        try:
            descriptor, probe = tempfile.mkstemp(
                prefix=".research-vault-manifest-probe-", dir=parent
            )
            os.close(descriptor)
            os.unlink(probe)
        except OSError as error:
...
def publish_outputs(
    ...
    try:
        descriptor, index_name = tempfile.mkstemp(
            prefix="research-vault-publish-index-"
        )
    except OSError as error:
