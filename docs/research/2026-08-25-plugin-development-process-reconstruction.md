...
01. **Wayfinder decision-ticket mapping** — the map issue's one-line-per-closed-ticket ledger;
    spec sections cite their tickets (issue #1; spec §3–§9).
02. **Spec-as-amendable-contract** — "amendable in place with dated notes… the goal state is a
    ...
    :200).
03. **Adversarial verification with counted findings** — every major artifact ships with its
...
06. **Planted-error drills + falsified-when clauses** — the gate's own failure modes seeded
    deliberately (spec:149), and the foundation names what falsifies it, including "the author
    routes around the vault" (spec:151).
07. **Audits as read-only artifacts** — finder lanes + fresh refuting verifiers, results committed
    as files with confirmed/refuted counts, "report only — nothing applied"
    (docs/research/validation-slice/).
08. **Research-note verdict/verification headers** — every evaluation note opens with its
    disposition so nobody re-runs it from scratch; verdicts name their instrument
    (product-landscape/README.md).
09. **Vendored-fork provenance** — pinned commit, license, per-file provenance headers,
