...
- Step 1/2 (failing tests, both sides): both sides have a test that pinned the defect. Side (a)
  ...
  adoption branch.
- Step 3 (side a): `hashes.append("unresolved")` is deleted at `knowledge_harness/__main__.py:233`
  ...
  `knowledge_harness/__main__.py:254` is untouched — it is driven by `local_path is None`, which the
  deleted line never influenced.
- Step 4 (side b): the brief's regex is present verbatim at both adoption sites,
  `knowledge_harness/verify.py:213` (candidate-snapshot plane) and `knowledge_harness/verify.py:225`
  (live-file plane). `re` was already imported at `knowledge_harness/verify.py:11`. A repo-wide grep
...
`knowledge_harness/verify.py:213` turns only `test_ack_hash_rejects_placeholder_fixity_candidate_snapshot`
red, reverting only `knowledge_harness/verify.py:225` turns only `..._live_file` red. A third lens
corroborated it independently by line coverage: the live-file test executes
`knowledge_harness/verify.py:216-227` and never 205-215; the snapshot test executes 197-215 and never
216-227. The controller does not need to re-run this.

## Strengths

- **Side (a) is exactly one deleted line.** `knowledge_harness/__main__.py:233` loses only the
...
- **Both adoption sites are guarded, and neither guard is redundant.** The per-site discrimination
  ...
  `_citekey_hash` (`knowledge_harness/verify.py:250-252`), and the two tests differ only in that
  keyword argument.
...
and matches acks by hash (`knowledge_harness/inbox.py:692-699`), so with `"aa11" != "aa11" * 16` the
...
(dropping the `outcome.result is Result.MATCHED or` short-circuit at `knowledge_harness/verify.py:757-758`
and feeding `effective` at `knowledge_harness/verify.py:1033`), this test FAILS at base and PASSES at
...
in principle enter a dedup key at `knowledge_harness/verify.py:855-870`, but that arm cannot fire in
this test: it runs `run_verify(..., network=False)`, and `_offline_network_outcomes`
(`knowledge_harness/verify.py:687-703`) emits only "outage — network disabled" outcomes with no
...
**`knowledge_harness/verify.py:243` — docstrings one level up still describe adoption as
...
(`knowledge_harness/verify.py:400`). Adoption is now shape-conditional: a note declaring a malformed
...
`knowledge_harness/verify.py::func/_citekey_hash::isinstance(first, str) and first -> isinstance(first, str) or first`,
...
**`knowledge_harness/verify.py:213` — "the adoption guard is now a verbatim six-line block duplicated
...
`git show 1fd3f8a:knowledge_harness/verify.py` shows the adoption block, including the predicate
