...
>
> **Sequencing, ruled 2026-09-01:** build now, not after `writing-specs`. `writing-requirements` is written here with `brainstorming` as inspiration, not derived from it — so it never merges upstream, was never a drift surface, and waiting for `writing-specs` buys nothing. The two are not siblings and are not required to match; each carries what its own altitude needs. **The gate on building is R27, not sequencing** — four corpora remain unapplied and their leads predicted three later-confirmed defects.
>
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------- |
| `brainstorming` — obra/superpowers, MIT | **Inspiration, not a base.** The idea that a phase carries a hard gate, a numbered checklist, a one-way ratchet, an anti-pattern table and a named handoff ritual. Text is written here, not lifted: the classifier is dropped — Spike/Bounded/Architectural classifies code, and "bounded" is defined by whether the flow already exists in the repo, which a need has no equivalent of — the write moves to the top of the loop per R8, and the anti-pattern rows are rewritten because upstream's argue against R2's decline. The **ratchet** is kept and retargeted: a started run never downgrades to a decline; a decline reverses when an answer shows the pre-flight was wrong. Read at **v6.2.0**, the version installed on both harnesses and the one carrying everything taken. The retargeted ratchet is the exception: 6.2.0 has no paths and nothing to ratchet between, so that idea comes from v6.3.0's path system and is not used as upstream states it. Recorded for the credit, not as a pin — nothing here is tracked | The shape of a process skill                            |
