...
**Roadmap authority note (2026-08-26): the wayfinder map — issue #33, "Report workflow and post-slice roadmap" — now owns the sequence.** Where this block and the map's tickets disagree, the map governs. The standing sequence below remains the 2026-08-25 record; the case-study selection was retargeted to Summary fidelity on 2026-08-28. **Standing sequence (re-decided 2026-08-25, with both gates clear):** this slice runs first, with the author — it is the critical path to validation and the author-in-the-loop work. After the slice closes, the post-slice improvement queue opens immediately, and Plan W (`2026-08-24-plan-w-quality-tail.md`) then runs in its peer session. This serialization is the author's scheduling preference, not a technical dependency or a change to Plan W's non-gating charter. The queue's contents live in five homes: the polish ledger (`docs/research/validation-slice/2026-08-22-skills-layer-audit.md`), the open tracker issues (#16, #19–#21, #24–#27), the standing recommendations register (`docs/product-landscape/2026-08-25-coding-companion-plugins-comparison.md`), this plan's Phase 5 pre-registered pilots, and spec §10's triggered deferrals. The queue becomes a plan (or plans) authored at slice close, against the slice's findings — not before: a plan authored now would be written against a stale evidence set, and the polish ledger's own extraction clause and the absorption pass's triggers already prescribe post-slice authoring. Plan W is followed by the read-only ponytail audit; architecture deepening waits for both the slice evidence and that audit. The author-scheduled wayfinder work (vault↔repo seam, then workload maps) is independent of this sequence.
...
- [ ] Admit this repo's research corpus into Zotero: the web sources under `docs/research/` (~40 cited URLs), the `sources/` PDFs. **This is the corpus-to-vault migration the release gate names** — PDFs go to Zotero storage (§2 boundary), not the vault.
...
- [x] **Question selected (2026-08-28, superseding `trust-gates-prior-art`): Summary fidelity** — “Which failure modes make LLM-authored source Summaries unfaithful or unstable, and which low-cost checks catch them when first written or regenerated?” The former 2026-08-22 `trust-gates-prior-art` selection returns to the case-study bank; its prior-art note remains a reference, not this slice's baseline. Then `project-flow` start: frame the Summary-fidelity question — question / scope / source types / success criteria — using the pre-slice digest and full-text rulings as the baseline; the deliverable is the rigorous answer plus the delta.
