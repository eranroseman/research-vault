...
The whole-library bibliography auto-export is a human-only wizard step of the same class: give the person the vault's `system/bibliography.json` target path and have them add an auto-export in BBT Preferences with whole-library scope, the Better CSL JSON translator, and keep updated enabled. Doctor does not observe or report on this file. Never register an auto-export for them.
