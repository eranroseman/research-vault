...
**Six requirements rest on sources never read directly.** R5 (Willis; Pew), R6 and R13 (Fitzpatrick, *The Mom Test*), R9 (Beyer and Holtzblatt), and R16 and R17 (Gilb's Planguage, read through a mirror). Each is reported by an agent or a mirror rather than quoted from the text. Nothing in the set is known to be wrong because of it, and R13 already carries the specific warning that its quoted phrasing came from a report; but the six are the largest remaining evidence gap and they are named here rather than left implied by absence from the list above.
