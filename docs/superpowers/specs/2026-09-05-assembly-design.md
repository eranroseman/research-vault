...
The register's linter checks three things: every `decided_in` resolves; every `floor_failed` resolves to a numbered must inside its `decided_in`; and **on an `adopt` or `adapt` row**, every `component` resolves to an entry in one of the four class registries (§6). That third check is what replaces holding a copy — the register and the registries are proved to agree rather than one restating the other.
...
**The check runs on `adopt` and `adapt` rows only, and that scope is load-bearing rather than an exemption.** The four registries are the four *third-party* classes, so what a hit attests is that we are using someone else's component. A `build` row names something we wrote — by construction in none of them. A `gap`, `open` or `reject` row may carry no `component` at all, which this section explicitly permits. Applied to every row the rule would reject four of the six values in its own `disposition` column, including the `build` verdict the entire build bar exists to govern. Registration proves adoption; it was never evidence about anything else.
...
**The fusion has already produced the defect this separation is meant to prevent, in the probe that ships today.** `doctor()` writes the vault tree and then reports on it, so `tree` can return `UNMATCHED` only when the repair *raised*: drift in the tree is repaired away before it is observed. That is not a hypothetical argument for `--check-only`, it is a live instance of it, and it means the tree probe cannot falsify the thing it names.
...
23. **Also found, and out of scope for this spec** — recorded here only so they are not lost, each needing its own issue rather than a lane: the mutation gate has never evaluated a mutant, because its only firing condition is a pull request and this repository has none; the OKF pin is written twice, in `quality.yml` and in ADR 0001, with nothing comparing the copies; scaffold spawns roughly three git subprocesses per seeded file, which the seeded `.obsidian/` multiplies; and the CRAP ceiling currently fails on two functions, `_bump_generated` (43.1) and `check_metadata` (37.0), against a limit of 30.
