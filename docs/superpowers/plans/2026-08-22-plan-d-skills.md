...
- **Semantic Scholar as a named source (grafted 2026-08-22)**: check what API the vendored fork already speaks; where it lacks them, add S2 public-REST legs through the existing webapi polite-pool (mailto etiquette; optional free key via env, never stored): paper search, citations, references, and recommendations — the citation-graph legs also serve `project`'s gap analysis ("what does this admitted paper cite that the vault lacks?"). Read-only outbound lookups, same class as the registry checks. Explicitly NOT via pyzotero's MCP server (consumed-MCP rejection stands; its authorize flow is a write path). Results cross-checked against the bibliography before presentation (registry-first dedup).
