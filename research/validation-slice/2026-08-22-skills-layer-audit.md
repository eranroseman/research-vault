...
12. *(prose-vs-mechanism audit 2026-08-24)* **synthesis-shape lint**: mechanize synthesis-conventions' three deterministic rules (2+-source threshold, min-link discipline, index registration) as a warn-tier lint in `lint_screening_state`'s shape; the skill keeps only the judgment half and drops its lines 14–24.
...
14. *(same audit)* **Shared-contract dedup**: the four four-state tables keep only their per-surface delta column (rides the RED-gated dedup decision); the 0/1/2/3 exit-code contract moves to one `--help` epilog with skill pointers; the six "never hand-write" statements collapse to one glossary statement + pointers.
