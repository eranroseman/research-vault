...
One naming detail worth knowing before installing both: obra's *plugin* is `elements-of-style`, but its *skill* is also `writing-clearly-and-concisely`. Different plugin names, so no plugin collision — but identical skill names, which per #89 means two catalog entries the model must choose between. The two are not interchangeable in content: softaworks ships the book as five chapter files plus a `signs-of-ai-writing.md`; obra ships a single `elements-of-style.md`.
