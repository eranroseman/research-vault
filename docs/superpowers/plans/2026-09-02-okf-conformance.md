...
- [ ] `skills/synthesis-conventions/SKILL.md` (D11): add the frontmatter sentence — new synthesis notes route through `system/templates/synthesis.md`, substituting `{{TITLE}}`/`{{ACTOR}}`/`{{NOW}}` (actor per §7: `human:<id>` or `<producer>/<version>`; time via `python3 -c "from research_vault.literature_notes import generated_at_now; print(generated_at_now())"`).
